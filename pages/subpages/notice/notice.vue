  ...
  >
    <view class="content">
      <z-paging v-if="true" ref="paging" v-model="dataList" @query="queryList">
        <view class="notice-item" v-for="message in dataList" :key='message.id'
          style="margin-bottom: 32rpx;"
          @click="$Router.push(`/pages/subpages/notice/detail?id=${message.id}`)"
        >
          <LineBox title='消息标题'>
            <view>
            ...
            <u-divider></u-divider>
            <view class="footer" style="padding-top: 10rpx;display: flex;justify-content: flex-end;">
              2022年1月1日 12:00
            </view>
          </LineBox>
        </view>
